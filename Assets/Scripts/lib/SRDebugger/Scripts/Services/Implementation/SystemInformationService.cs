	...
	public class StandardSystemInformationService : ISystemInformationService
	{
		...
		private void CreateDefaultSet()
		{
			...
			_info.Add("Unity", new[] {
				...
				Info.Create("Genuine",
					"{0} ({1})".Fmt(Application.genuine ? "Yes" : "No", Application.genuineCheckAvailable ? "Trusted" : "Untrusted")),
			...
			var manifestDict = cloudBuildManifest != null ? Json.Deserialize(cloudBuildManifest.text) as Dictionary<string, object> : null;
			...
			_info.Add("Graphics", new[] {
				...
				Info.Create("Render Textures",
					"{0} ({1})".Fmt(SystemInfo.supportsRenderTextures ? "Yes" : "No", SystemInfo.supportedRenderTargetCount)),
